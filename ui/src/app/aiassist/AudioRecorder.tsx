...
export function AudioRecorder({ ...props }: AudioRecorderProps) {
  ...
  const startRecording = async () => {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      ...
      notify.error('Microphone access is not supported in this environment.')
      return
    }
    ...
    const stream = await navigator.mediaDevices.getUserMedia({ audio: true }).catch((e) => {
      ...
      notify.error('Error accessing microphone: ' + e.message)
    })
    if (!stream) return
  ...
  const stopRecording = () => {
    ...
    mediaRecorder.ondataavailable = (e) => {
      ...
      if (props?.audioChanged) {
        props.audioChanged(e.data)
      }
    }
    mediaRecorder.stop()
